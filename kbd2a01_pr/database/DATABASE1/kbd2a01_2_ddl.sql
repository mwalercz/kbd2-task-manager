...
CREATE TRIGGER PROJECTS_BIR_TRG BEFORE INSERT ON PROJECTS FOR EACH ROW WHEN (NEW.PROJECT_ID IS NULL)
BEGIN
  SELECT PROJECTS_SEQUENCE.NEXTVAL INTO :NEW.PROJECT_ID FROM DUAL;
END;
/

CREATE TRIGGER TASKS_BIR_TRG BEFORE INSERT ON TASKS FOR EACH ROW WHEN (NEW.TASK_NO IS NULL)
BEGIN
  SELECT MAX(TASK_NO) + 1 INTO :NEW.TASK_NO FROM TASKS WHERE PROJECT_ID = :NEW.PROJECT_ID;
END;
/

CREATE TRIGGER TASKS_BUR_TRG BEFORE UPDATE ON TASKS FOR EACH ROW WHEN (NEW.CREATED_DATE <> OLD.CREATED_DATE)
BEGIN
